"""Two VPN slots: the primary egress (SOCKS on 1370) and the secondary one (Opera or Tor).

Nova keeps two tunnels up at once. The **primary** is one of Nova's own userspace backends on
127.0.0.1:1370 -- Cloudflare AWG, Cloudflare MASQUE or Proton AWG. The **secondary** is the reserve:
Opera's HTTP proxy on 127.0.0.1:1371 (region EU or US) or Tor (SOCKS 1375, HTTP CONNECT 1378).
...
__all__ = [
    ...
    "PRIMARY_AUTO", "PRIMARY_CF_AWG", "PRIMARY_CF_MASQUE", "PRIMARY_PROTON", "PRIMARY_CUSTOM", "PRIMARY_CF_WARP",
    "PRIMARY_CHOICES", "PRIMARY_MENU_LABELS", "PRIMARY_PILL_LABELS",
    ...
    "SECONDARY_AUTO", "SECONDARY_OPERA", "SECONDARY_TOR", "SECONDARY_MODES", "SECONDARY_MENU_LABELS",
...
# Never offered as a slot choice: a Custom profile is pinned in «Профили», and warp-cli is the last
# step of «Авто». They exist so the indicator and the menu can name what is actually running.
...
def selection_for_primary(kind, country=""):
...
def primary_indicator_label(backend, profile_id, connected, choice_kind=PRIMARY_AUTO):
    ...
    `backend` is WarpManager.active_backend ("awg", "masque", "cloudflare"); `profile_id` the full
    `group/name`. A Proton exit names its country when the profile name carries one.
    """
...
SECONDARY_TOR = "tor"
SECONDARY_MODES = (SECONDARY_AUTO, SECONDARY_OPERA, SECONDARY_TOR)
SECONDARY_MENU_LABELS = {SECONDARY_AUTO: "Авто", SECONDARY_OPERA: "Opera", SECONDARY_TOR: "Tor"}
OPERA_REGIONS = ("EU", "US")

# The settings row offers four pills: the Opera region is part of the choice there.
...
def normalize_secondary_block(block):
    """The `secondary` block of routing_settings.json: `{"mode": auto|opera|tor}`.

    The Opera region stays in the top-level `opera_region` and the Tor entry in `tor.entry`,
    where earlier versions already keep them; this block only says which of the two runs.
    """
...
def secondary_choice(settings):
    """routing settings -> {"mode", "opera_region", "tor_entry"}."""
...
def secondary_attempts(choice, last=None):
    """The order the secondary slot walks: [(kind, region_or_entry), ...].
    ...
    is one, otherwise Tor's own «Авто» walk.
    """
    choice = choice if isinstance(choice, dict) else {}
    mode = normalize_secondary_mode(choice.get("mode"))
...
def secondary_indicator_label(kind, region=""):
    """EU / US for Opera (Opera's runtime label may add `/WARP`), TOR for Tor."""
    if str(kind or "").strip().lower() == SECONDARY_TOR:
        return "TOR"
...
def make_egress_state(*, primary_up, primary_kind="", primary_label="", primary_country="",
                      secondary_up, secondary_kind=SECONDARY_OPERA, secondary_label="", now=None):
    ...
    secondary_kind = SECONDARY_TOR if str(secondary_kind or "").lower() == SECONDARY_TOR else SECONDARY_OPERA
    return {
...
_DEFAULT_STATE = {
    ...
    "primary": {"up": False, "kind": "", "label": "", "country": "", "foreign": False,
                "socks_port": PRIMARY_SOCKS_PORT},
    "secondary": {"up": False, "kind": SECONDARY_OPERA, "label": "", "http_port": OPERA_HTTP_PORT, "socks_port": 0},
}
...
def normalize_egress_state(payload, now=None):
    ...
    kind = SECONDARY_TOR if str(secondary.get("kind") or "").lower() == SECONDARY_TOR else SECONDARY_OPERA
...
def secondary_pac_tokens(kind, http_port=None, socks_port=None):
    """PAC tokens for the secondary. Tor gets SOCKS5 first (browsers) and PROXY for WinINET."""
    if str(kind or "").strip().lower() == SECONDARY_TOR:
...
def secondary_http_attempt(state, timeout=3.0, first_byte_timeout=2.4):
    ...
    `egress` says what it really is, for the log.
    """
...
def remember_secondary(base_dir, kind, region="", entry="", now=None):
    """Record the secondary that just carried traffic."""
    kind = SECONDARY_TOR if str(kind or "").lower() == SECONDARY_TOR else SECONDARY_OPERA
    record = {
        "kind": kind,
