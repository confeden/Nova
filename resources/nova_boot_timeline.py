...
def worker_started(name, requested_delay=0.0):
    ...
    Запрошенная задержка кладётся рядом, иначе «проснулся на +16 с» невозможно
    отличить от «так и было задумано».
    """
    began = elapsed_ms()
    _add("worker", name, began, None, "%.0f" % (float(requested_delay or 0.0) * 1000.0))
    return began


def _worker_lag(record):
    """На сколько воркер опоздал против запрошенной задержки, мс."""
    try:
