...
try:
    ...
    def is_vpn_active_func():
        ...
        try:
            ...
            keywords = ["vpn", "wireguard", "openvpn", "tun", "tap", "zerotier", "tailscale", "secu", "fortinet", "cisco", "hamachi", "radmin", "amnezia", "warp", "cloudflare"]
    ...
    def get_line_tag(line):
        ...
        if "[StrategyChecker]" in line: return "info"
    ...
    def ensure_log_window_created():
        ...
        def open_telegram_chat():
            ...
            try:
                # Пытаемся открыть через протокол tg://
        ...
        log_text_widget = tk.Text(content_frame, font=("Consolas", 11, "bold"), bg="#000000", fg="#E0E0E0", yscrollcommand=scrollbar.set, bd=0, highlightthickness=0)
    ...
    def log_print(message):
        ...
        if root:
            root.after(0, lambda: _safe_log_insert(message))
    ...
    def _safe_log_insert(string):
        if log_window and tk.Toplevel.winfo_exists(log_window):
            try:
                ...
                tag = "normal"
                if "[StrategyChecker]" in string: tag = "info"
                ...
                if auto_scroll_enabled: log_text_widget.see(tk.END)
            except: pass

    class RedirectText(object):
        def write(self, string):
            # FIX: Check if root exists safely
            try:
                if root:
        ...
        while not is_closing:
            try:
                ...
                try:
                    ...
                    log_func(f"[StrategyChecker] Ошибка подготовки ядра: {e}")
                ...
                if process and process.poll() is not None and is_service_active:
                     log_func(f"[StrategyChecker] КРИТИЧЕСКОЕ ПРЕДУПРЕЖДЕНИЕ: Основной процесс winws.exe упал (код {process.poll()}). Перезапуск...")
                ...
                if not domains:
                    log_func("[StrategyChecker] rkn.txt пуст или отсутствует. Ожидание...")
                    time.sleep(60)
                    continue
                ...
                def check_strat(strat_args, port_start, target_domains=None, rate_limiter=None):
                    if is_closing or not is_service_active: return None  # Возвращаем None при прерывании
                    ...
                    for arg in strat_args:
                        if '.bin' in arg and '=' in arg:
                            try:
                                ...
                                log_func(f"[StrategyChecker] Пропуск: отсутствует файл {val}")
                                return None
                            except:
                                pass
                    ...
                    try:
                        ...
                        # Используем тестовый exe, если он есть (создается при старте воркера)
                        final_exe = test_exe_path if os.path.exists(test_exe_path) else exe_path
                        ...
                        for attempt in range(2):
                            ...
                            try:
                                ...
                                log_func(f"[StrategyChecker] WinWS завис ( > 1000сек). Принудительное завершение.")
                                try: subprocess.run(["taskkill", "/F", "/PID", str(proc.pid)], creationflags=subprocess.CREATE_NO_WINDOW, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                                except: pass
                                return None
                            ...
                            if elapsed > 1000:
                                log_func(f"[StrategyChecker] ВНИМАНИЕ: Стратегия проверялась {elapsed:.1f} сек!")
                            
                            # Логируем только если это не штатная остановка
                            if proc.returncode != 0:
                                log_func(f"[StrategyChecker] WinWS упал (код {proc.returncode}). Порт: {port_start}. Попытка {attempt+1}/2")
                                if attempt == 0:
                                    if stderr_out: log_func(f"[StrategyChecker] Ошибка WinWS: {stderr_out.strip()}")
                                if attempt == 1:
                                    log_func(f"[StrategyChecker] Стратегия вызывает сбой процесса. Пропускаем.")
                                    return 0
                            log_func(f"[StrategyChecker] WinWS упал (код {proc.returncode}). Порт: {port_start}. Попытка {attempt+1}/2")
                            if attempt == 0:
                                # log_func(f"[StrategyChecker] Аргументы сбоя: {' '.join(final_args)}")
                                if stderr_out: log_func(f"[StrategyChecker] Ошибка WinWS: {stderr_out.strip()}")
                            if attempt == 1:
                                log_func(f"[StrategyChecker] Стратегия вызывает сбой процесса. Пропускаем.")
                        ...
                        log_func(f"[StrategyChecker] Ошибка теста: {e}")
                        return None
                    finally:
                        if proc:
                            try: subprocess.run(["taskkill", "/F", "/PID", str(proc.pid)], creationflags=subprocess.CREATE_NO_WINDOW, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                ...
                def check_strat_threaded(strat_args, target_domains=None):
                    if is_closing or not is_service_active: return None
                    ...
                    try:
                        with BACKGROUND_SEM:
                            return check_strat(strat_args, p_start, target_domains, rate_limiter=BACKGROUND_RATE_LIMITER)
                    finally:
                ...
                # === НОВОЕ: Функция мутации стратегий ===
                def mutate_strategy(args, bin_files):
                    if not args: return []
                    # FIX: Sanitize args to prevent crash if malformed (e.g. nested lists)
                    args = [a for a in args if isinstance(a, str)]
                    
                    mutations = []
                    base_args = [a for a in args if not a.startswith("--wssize") and not a.startswith("--dpi-desync-wssize")]
                    
                    # 1. TTL Enforcement (Max 11)
                    ttl_fixed = False
                    for i, arg in enumerate(base_args):
                        if "--dpi-desync-ttl=" in arg:
                            try:
                                val = int(arg.split('=')[1])
                                if val > 11:
                                    base_args[i] = "--dpi-desync-ttl=11"
                                    ttl_fixed = True
                            except: pass
                    if ttl_fixed: mutations.append(list(base_args))

                    # 2. Bin Injection / Swapping (Support Dual Bins)
                    # Helper to get random bin
                    def get_rnd_bin(is_quic=False):
                        if not bin_files: return None
                        if is_quic:
                            c = [b for b in bin_files if "quic" in b.lower() or "initial" in b.lower()]
                            return random.choice(c) if c else random.choice(bin_files)
                        else:
                            c = [b for b in bin_files if "tls" in b.lower() or "clienthello" in b.lower()]
                            return random.choice(c) if c else random.choice(bin_files)

                    has_new = "--new" in base_args
                    
                    # Если нет bins, пробуем добавить
                    has_fake_tls = any("--dpi-desync-fake-tls" in a for a in base_args)
                    has_fake_quic = any("--dpi-desync-fake-quic" in a for a in base_args)
                    
                    if not has_fake_tls:
                        b = get_rnd_bin(is_quic=False)
                        if b:
                            m = list(base_args)
                            # Вставляем перед --new если есть
                            idx = m.index("--new") if "--new" in m else len(m)
                            m.insert(idx, f"--dpi-desync-fake-tls={sanitize_fake_path(b)}")
                            mutations.append(m)
                            
                    if has_new and not has_fake_quic:
                        b = get_rnd_bin(is_quic=True)
                        if b:
                            m = list(base_args)
                            m.append(f"--dpi-desync-fake-quic={sanitize_fake_path(b)}")
                            mutations.append(m)

                    # 3. Split Position Mutation
                    split_positions = ["1", "2", "3", "midsld", "midsld+1", "host+1"]
                    current_splits = [a for a in base_args if "--dpi-desync-split-pos=" in a]
                    for sp in current_splits:
                        curr_val = sp.split('=')[1]
                        for new_val in split_positions:
                            if new_val != curr_val:
                                m = [a if a != sp else f"--dpi-desync-split-pos={new_val}" for a in base_args]
                                mutations.append(m)

                    # 4. Repeats Mutation (2..6)
                    current_repeats = [a for a in base_args if "--dpi-desync-repeats=" in a]
                    for cr in current_repeats:
                        try:
                            val = int(cr.split('=')[1])
                            candidates = [val-1, val+1]
                            for c in candidates:
                                if 2 <= c <= 6:
                                    m = [a if a != cr else f"--dpi-desync-repeats={c}" for a in base_args]
                                    mutations.append(m)
                        except: pass
                ...
                if is_vpn_active_func():
                    log_print("[StrategyChecker] Обнаружен VPN, проверка IP отложена.")
                    time.sleep(5)
                    continue
                ...
                try:
                    ...
                    # log_func(f"[StrategyChecker] Не удалось определить IP: {e}. Повтор через 60 сек.")
                    time.sleep(60)
                    continue
                ...
                if not need_check:
                    if not state.get("completed", False):
                        # Force check if not explicitly marked as completed
                        # But also check specific flags to be sure? 
                        # Actually, "completed" is the master flag now.
                        if IS_DEBUG_MODE: log_func(f"[StrategyChecker-Debug] Force check: completed=False")
                        need_check = True
                    else:
                        # Double check sub-flags just in case master flag was set prematurely?
                        # No, trust the master flag to avoid loops.
                        pass
                ...
                if not need_check:
                    ...
                    if now_ts - last_log > 3600:
                         ...
                         log_func(f"[StrategyChecker] Проверка не требуется (все стратегии актуальны). Ожидание смены IP или истечения срока давности (7 дней).")
                ...
                def prepare_service_tasks(service_key, json_path, test_domains, state_key):
                    ...
                    # Добавляем остальные стратегии из файла
                    try:
                        with open(json_path, "r", encoding="utf-8") as f:
                            raw_data = json.load(f)
                            # === MODERN FORMAT: {"version": "...", "strategies": [...]} ===
                            if isinstance(raw_data, dict) and "strategies" in raw_data:
                                for s in raw_data["strategies"]:
                                     if not service_strategies or (service_strategies and str(s.get("args")) != str(service_strategies[0]["args"])):
                            ...
                            # === LEGACY DICT FORMAT: {"name": [...], ...} ===
                            elif isinstance(raw_data, dict):
                                for k, v in raw_data.items():
                                    if k == "version": continue
                                    # Пропускаем, если это дубликат текущей активной (уже добавлена)
                                    if not service_strategies or (service_strategies and str(v) != str(service_strategies[0]["args"])):
                                         service_strategies.append({"name": k, "args": v})
                    except: pass
                ...
                # log_func(f"[StrategyChecker] Проверка {len(clean_hot_domains)} посещаемых доменов...")
                ...
                if needed > 0:
                    ...
                    if cold_candidates:
                        # log_func(f"[StrategyChecker] Добираем {needed} доменов из RKN (проверка {len(cold_candidates)} кандидатов)...")
                        alive_cold = filter_alive_domains(cold_candidates, needed)
                
                domains_for_general = alive_hot + alive_cold
                
                log_func(f"[StrategyChecker] Тестирование на {len(alive_hot)} заблокированных доменах из истории посещений и {len(alive_cold)} доменов из списка rkn")
                ...
                # Добавляем в общий пул, если нужна проверка
                if state["general_score"] <= 0 or not state.get("hard_checked", False):
                    general_args = []
                    try:
                        with open(strat_path, "r", encoding="utf-8") as f:
                            data = json.load(f)
                ...
                if all_tasks:
                    ...
                    log_func(f"[StrategyChecker] Задачи: {len(all_tasks)}. Потоков: {STRATEGY_THREADS}")
                    ...
                    if skipped_count > 0:
                         log_func(f"[StrategyChecker] Пропущено {skipped_count} стратегий (уже проверены). Осталось: {len(task_queue)}")
                    
                    priority_tasks = set() # Init empty set to prevent NameError
                    restart_needed = False # Track if any phase requires a restart
                    
                    service_baselines = {}
                    if state["general_score"] >= 0:
                        service_baselines["general"] = state["general_score"]
                    for svc in ["general", "youtube", "discord", "cloudflare", "whatsapp", "telegram"]:
                        key = f"{svc}_score"
                        if state.get(key, -1) >= 0:
                            service_baselines[svc] = state[key]
                    

                    log_func(f"[StrategyChecker] Запуск проверки {len(task_queue)} стратегий ({STRATEGY_THREADS} потоков)...")
                    ...
                    try:
                        ...
                        while (task_queue or active_futures) and not is_closing:
                            # Fill slots
                            while len(active_futures) < STRATEGY_THREADS and task_queue:
                                task = task_queue.pop(0)
                                svc, strat, doms, path = task
                                
                                # Skip if baseline already met for "Current" strategies
                                # (Avoids re-testing the same strategy on restart if we have a valid score)
                                is_curr_strat = strat["name"].startswith("Current ") or strat["name"].startswith("Текущая ")
                                if is_curr_strat and service_baselines.get(svc, -1) >= 0:
                                    # We already know the score for this service's current strategy
                                    continue
                                    
                                fut = executor.submit(check_strat_threaded, strat["args"], doms)
                                active_futures[fut] = task

                            # Abort if service paused
                            if not is_service_active:
                            ...
                            while audit_running_event.is_set():
                                 time.sleep(1) # Yield to Periodic Checker
                                 
                            # Loop wait to allow aborting mid-wait
                            done = set()
                            while not done and (task_queue or active_futures) and not is_closing:
                                if not is_service_active: 
                                    aborted_by_service = True
                                    break # Abort check
                                
# Progress Logging (Отключено по просьбе, чтобы вернуть детальный лог)
                                # if time.time() - last_progress_log > 5:
                                #     remaining = len(task_queue) + len(active_futures)
                                #     completed = total_tasks_count - remaining
                                #     log_func(f"[StrategyChecker] Прогресс: {completed}/{total_tasks_count}")
                                #     last_progress_log = time.time()

                                done, _ = concurrent.futures.wait(active_futures.keys(), timeout=1, return_when=concurrent.futures.FIRST_COMPLETED)
                            
                            if aborted_by_service: break
                            ...
                            for fut in done:
                                ...
                                try:
                                    ...
                                    # === ВОССТАНОВЛЕНИЕ ЛОГИКИ ВЫВОДА (Old Style) ===
                                    total_cnt = len(doms)
                                    s_name = strat["name"]
                                    bl = service_baselines.get(svc, 0)

                                    # Проверка на "Текущую" стратегию
                                    ...
                                    if is_current:
                                        ...
                                        save_state() # FIX: Persist 'Current' score immediately!
                                        
                                        # Текущую выводим всегда
                                        log_func(f"[StrategyChecker] {s_name}: {sc}/{total_cnt}")
                                    else:
                                        # User Request: Show ALL results even if 0
                                        # Also adding service name for clarity
                                        log_func(f"[StrategyChecker] {s_name} ({svc}): {sc}/{total_cnt}")

                                    all_strategy_results.append((sc, strat, svc))
                                    
                                    # Если нашли что-то реально лучше текущего baseline — подсвечиваем, и обновляем планку
                                    if not is_current and sc > bl:
                                         log_func(f"   >>> Лучше текущей! ({sc} > {bl}) применяется как новая для {svc}...")
                                         
                                         service_baselines[svc] = sc
                                         
                                         # FIX: Immediately save as new default to apply it "on the fly"
                                         try:
                                             s_file = os.path.join(get_base_dir(), "strat", f"{svc}.json")
                                             if svc == "general": s_file = os.path.join(get_base_dir(), "strat", "strategies.json")
                                             
                                             curr_data = load_json_robust(s_file)
                                             
                                             if svc == "general":
                                                  # strategies.json is a Dict { "general": [args, ...], ... }
                                                  if not isinstance(curr_data, dict): curr_data = {}
                                                  if "general" not in curr_data: curr_data["general"] = []
                                                  # Insert only args for general
                                                  curr_data["general"].insert(0, strat["args"])
                                                  save_json_safe(s_file, curr_data)
                                             else:
                                                  # Service files can be:
                                                  # 1. List [ {name, args}, ... ] (Intermediate)
                                                  # 2. Dict { "strategies": [...], "version": ... } (Modern)
                                                  # 3. Dict { "strat1": [...], ... } (Legacy)
                                                  
                                                  # If it's a dict, we need to handle it carefully to not lose data
                                                  if isinstance(curr_data, dict):
                                                      if "strategies" in curr_data:
                                                          # Modern format: Insert into list
                                                          if not isinstance(curr_data["strategies"], list): curr_data["strategies"] = []
                                                          curr_data["strategies"].insert(0, strat)
                                                      else:
                                                          # Legacy/Mixed format conversion
                                                          existing_strats = []
                                                          for k, v in curr_data.items():
                                                              if k == "version": continue
                                                              existing_strats.append({"name": k, "args": v})
                                                          
                                                          existing_strats.insert(0, strat)
                                                          curr_data = {
                                                              "version": CURRENT_VERSION,
                                                              "strategies": existing_strats
                                                          }
                                                  elif isinstance(curr_data, list):
                                                      curr_data.insert(0, strat)
                                                  else:
                                                      # Unknown/Empty -> New List
                                                      curr_data = [strat]
                                             ...
                                             save_state() # FIX: Persist new baseline immediately!

                                             log_func("   Применены новые стратегии")
                                             if root: root.after(0, perform_hot_restart)
                                         except Exception as e:
                                             log_func(f"   [Error] Не удалось сохранить стратегию: {e}")
                                    
                                    # === SAVE PROGRESS ===
                                    completed_tasks.add(s_name)
                                    # Optimization: Save every 5 items or if queue empty
                                    if len(completed_tasks) % 5 == 0 or not task_queue:
                                    ...
                                    log_func(f"[StrategyChecker] Task Error: {e}")

                        # End of Phase 1 Loop
                    finally:
                        # Non-blocking shutdown if aborted, else blocking (or pool just dies)
                ...
                if aborted_by_service or (not is_service_active and not is_closing):
                    ...
                    if task_queue or active_futures:
                         log_func("[StrategyChecker] Процесс остановлен пользователем.")
                ...
                def sort_and_distribute_results(results_list, log_func=log_func):
                    # Sort by Score DESC
                    ...
                    for svc, items in grouped.items():
                        ...
                        if svc == "general":
                            # Special Logic for General:
                            # 1. Top 12 -> hard_X
                            # 2. Rest -> general.json
                            
                            log_func(f"[Sorter] {svc}: Список стратегий оптимизирован.")
                            
                        else:
                            # For Service: Just sort list in file
                            start_path = os.path.join(base_dir, "strat", f"{svc}.json")
                            try:
                                # Start with modern dict structure
                                new_strats_list = []
                                for sc, st in items:
                                    new_strats_list.append(st)
                                
                                # Read version from file or default?
                                ver = CURRENT_VERSION
                                
                                # Save
                                new_data = {"version": ver, "strategies": new_strats_list}
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=STRATEGY_THREADS) as executor:
                    # --- PHASE 3: EVOLUTION (3 STAGES) ---
                    evolution_stages = [1.0, 0.5, 0.25]
                
                    # Cache domains for reuse
                    # domains_for_general is available from scope
                    
                    for stage_idx, percentage in enumerate(evolution_stages):
                        if is_closing: break
                        
                        pool_desc = f"{int(percentage*100)}%"
                        log_func(f"[StrategyChecker] --- ЭТАП ЭВОЛЮЦИИ {stage_idx+1}/3 ({pool_desc}) ---")
                        ...
                        try:
                            ...
                            # FIX: If d is None or not dict, ensure it's a dict to check "general" key
                            if not isinstance(d, dict): d = {}
                            s_gen = []
                            # Hard
                            for i in range(12):
                                k = f"hard_{i+1}"
                                if k in d: s_gen.append({"name": k, "args": d[k]})
                            # General List
                            if "general" in d:
                                for idx, args in enumerate(d["general"]):
                                     s_gen.append({"name": f"gen_{idx}", "args": args})
                            
                            # Calculate count for general strategies based on percentage
                            count = int(len(s_gen) * percentage)
                            if count < 1 and s_gen: count = 1
                            strategies_to_evolve.extend([(s, "general") for s in s_gen[:count]])
                            
                            # Special
                            for svc in ["youtube", "discord", "cloudflare", "whatsapp", "telegram"]:
                                sp = os.path.join(base_dir, "strat", f"{svc}.json")
                                if os.path.exists(sp):
                                     s_spec = load_json_robust(sp, [])
                                     if isinstance(s_spec, list):
                                         count = int(len(s_spec) * percentage)
                                         if count < 1 and s_spec: count = 1
                                         strategies_to_evolve.extend([(s, svc) for s in s_spec[:count]])
                        except Exception as e:
                        ...
                        for strat, svc in strategies_to_evolve:
                            # Select domains
                            doms = domains_for_general if svc == "general" else prepare_service_tasks(svc, "", [], "").pop()[2] # Hacky: Use helper to get domains?
                            # Simplified: Use general domains for general, empty for special (let check_strat handle default?)
                            # Better: Re-use domains_for_general for all? No.
                            # Let's stick to simple logic: General -> General Domains. Special -> Special Domains.
                            ...
                            if svc != "general":
                                ...
                                if t_doms: target_doms = t_doms[:20] # Limit to 20 for speed in Evo
                            
                            mutations = mutate_strategy(strat['args'], bin_files)
                            for i, m_args in enumerate(mutations):
                                 evo_tasks.append((svc, {"name": f"{strat['name']}_M{i}", "args": m_args}, target_doms, ""))

                        if not evo_tasks: continue

                        log_func(f"[StrategyChecker] Эволюция: проверка {len(evo_tasks)} вариаций...")
                        
                        # 3. Check Loop (Inline)
                        evo_results = []
                        # Note: We reuse the SAME executor contextManager? 
                        # NO! We are INSIDE 'with ThreadPoolExecutor' block of Phase 1?
                        # YES, we are inside 'with executor'. So we can reuse 'executor'.
                        
                        active_evo = {}
                        q_evo = list(evo_tasks)
                        
                        better_found = False
                        
                        while (q_evo or active_evo) and not is_closing:
                            while len(active_evo) < STRATEGY_THREADS and q_evo:
                                t = q_evo.pop(0)
                                svc, strat, doms, _ = t
                                act_f = executor.submit(check_strat_threaded, strat["args"], doms)
                                active_evo[act_f] = t
                            ...
                            for f in done:
                                ...
                                try:
                                    sc = f.result()
                                    if sc is None: continue
                                    evo_results.append((sc, strat, svc))
                                    
                                    # FIX: Show Evo results for verification
                                    if sc > 0:
                                        log_func(f"[Evo] {strat['name']} ({svc}): {sc}")

                                    bl = service_baselines.get(svc, 0)
                                    if sc > bl:
                                         log_func(f"[Evo] >>> Улучшение для {svc}: {sc} (было {bl}) -> {strat['name']}")
                                         service_baselines[svc] = sc
                                         better_found = True
                                except: pass
                        ...
                        # 4. Sort Phase
                        if evo_results:
                             sort_and_distribute_results(evo_results)
                        
                        # 5. Instant Restart if better
                        if better_found:
                            log_func(f"[StrategyChecker] Эволюция нашла улучшения. Перезапуск ядра...")
                            if root: root.after(0, perform_hot_restart)

                    # FINISH
                    log_func("[StrategyChecker] Подбор полностью завершен.")
                    
                    # No Final Restart call here.
                    # (Old code had it, we removed it)
                ...
                log_func("[StrategyChecker] Цикл проверки завершен.")
                ...
                if "aborted_by_service" in str(e) or "Остановка сервиса" in str(e) or not is_service_active:
                     ...
                     log_func(f"[StrategyChecker] Ошибка рабочего цикла: {e}")
    ...
    def repair_windivert_driver(log_func=None):
        """
        Attempts to repair broken WinDivert driver installation (Fix for Code 177).
        """
        ...
        # 2. Add delay for SCM to clean up registry
        time.sleep(2.0)
        
        # 3. Restore driver files from backup if available (or assume they are present in bin)
    ...
    def _start_nova_service_logic(silent=False, restart_mode=False): # Added restart_mode
        ...
        def run_process():
            global process, is_closing, is_service_active, nova_service_status
                ...
                if is_cluttered:
                    ...
                    if ret == 6: # IDYES - Relocate
                        ...
                        try:
                            ...
                            # Launch new process
                            subprocess.Popen([target_exe] + sys.argv[1:], cwd=target_dir)
                            ...
                            ctypes.windll.user32.MessageBoxW(0, f"Ошибка при создании папки: {e}", "Ошибка", 0x10)
                            sys.exit() 
